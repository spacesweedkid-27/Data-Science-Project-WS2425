...
# Define STOP_WORDS
STOP_WORDS = {"wan", "na", "ta", "ca", "and", "the", "to", "of", "in", "a", "for", "on", "is", "it", "you", "are", "this", "with", "by", "i", "be", "as", "that", "have", "at", "so", "up", "out", "but", "from", "all", "just", "some", "can"}

# Define the function for n-gram frequency
...
def create_word_frequency_chart(n, lyrics_data):
    freq_data = get_ngram_frequencies(lyrics_data, n)
...
@dash.callback(
    ...
    Input('theme-store', 'data'),  # Handle theme change
    Input('lyrics-store', 'data')  # Get lyrics data from store
)
def update_chart(n, theme_data, lyrics_data):
    # Generate the chart with lyrics data
    fig = create_word_frequency_chart(n, lyrics_data)

    # Apply theme styles if theme_data is available
    if theme_data:
        if theme_data == "dark":
            fig.update_layout(
                plot_bgcolor="black", 
